...
function sip(){const a=$('#sip-amount'),y=$('#sip-years'),r=$('#sip-rate');if(!a||!y||!r)return;const av=$('#sip-amount-val'),yv=$('#sip-years-val'),rv=$('#sip-rate-val');const read=(e,d)=>Number.isFinite(parseFloat(e?.value))?parseFloat(e.value):d;
 const calc=()=>{const P=Math.max(0,read(av||a,5000)),yrs=Math.max(1,read(yv||y,15)),rate=Math.max(0,read(rv||r,12)),m=Math.round(yrs*12),mr=rate/1200;const fv=mr===0?P*m:P*((Math.pow(1+mr,m)-1)/mr)*(1+mr),invested=P*m,ret=Math.max(0,fv-invested);[['total-invested',invested],['expected-returns',ret],['total-value',fv]].forEach(([id,v])=>{const e=$('#'+id);if(e)e.textContent='₹'+fmt(v)});if(typeof Chart==='undefined'||!$('#sip-chart'))return;if(sipChart)try{sipChart.destroy()}catch(e){}const dark=document.documentElement.dataset.theme==='dark',tc=dark?'#b0aac8':'#555770',gc=dark?'rgba(255,255,255,.07)':'rgba(0,0,0,.06)',labels=[],investedData=[],val=[];for(let yr=1;yr<=yrs;yr++){const mm=yr*12;labels.push('Yr '+yr);investedData.push(P*mm);val.push(mr===0?P*mm:P*((Math.pow(1+mr,mm)-1)/mr)*(1+mr))}sipChart=new Chart($('#sip-chart').getContext('2d'),{type:'bar',data:{labels,datasets:[{label:'Total Invested',data:investedData,backgroundColor:'rgba(36,72,216,.7)',borderRadius:4},{label:'Expected Value',data:val,backgroundColor:'rgba(75,46,131,.85)',borderRadius:4}]},options:{responsive:true,maintainAspectRatio:false,scales:{x:{grid:{color:gc},ticks:{color:tc}},y:{beginAtZero:true,grid:{color:gc},ticks:{color:tc}}},plugins:{legend:{labels:{color:tc}}}}});window.sipChart=sipChart};
